...
class _ImageCropOcrWidgetState extends State<ImageCropOcrWidget> {
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      try {
        ...
        setState(() {
          _errorMessage = 'ファイルの読み込みに失敗しました。別の画像をお試しください。';
        });
      }
    } catch (e) {
      print('画像選択エラー: $e');
  ...
  void _setImageBytes(Uint8List imageBytes) {
    ...
    setState(() {
      ...
      _errorMessage = '画像が読み込まれました。必要に応じてトリミングやOCR解析を行ってください。';
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _errorMessage = 'モード判別に失敗しました。デフォルトの写真モードを使用します。';
        });
      }
    }
  }
  ...
  void _cropImage() {
    ...
    if (_isCropping) {
      print('トリミングが既に進行中です');
      setState(() {
        _errorMessage = 'トリミングが進行中です。しばらくお待ちください。';
      });
      return;
    }
      ...
      if (_selectedImageBytes != null && _userSpecifiedCropArea != null) {
        ...
        if (mounted) {
          setState(() {
            ...
            _errorMessage = '手動トリミング成功!';
          });
        }
      } else {
        setState(() {
          _isCropping = false;
          _errorMessage = 'トリミングに必要な情報が不足しています。';
        });
      }
    } catch (e) {
      print('手動トリミングエラー: $e');
  ...
  Future<void> _executeOcr() async {
    ...
    try {
      ...
      if (result['success'] == true && result['text'] != null) {
        ...
        if (_enableAutoDetect) {
          ...
          if (_isVocabularyDetected && _hasMathFormula) {
            setState(() {
              _errorMessage = '単語リストと数式が検出されました。適切な形式で整形しました。';
            });
          } else if (_isVocabularyDetected) {
            setState(() {
              _errorMessage = '単語リストが検出されました。自動的に整形しました。';
            });
          } else if (_hasMathFormula) {
            setState(() {
              _errorMessage = '数式が検出されました。TeX形式で出力しました。';
            });
          }
        }
      } else {
      ...
      setState(() {
        ...
        _errorMessage = 'OCR解析が完了しました。ウィジェットを閉じます...';
      });

      // 少し間を置いてから自動でウィジェットを閉じる
      ...
      setState(() {
        _isAnalyzing = false;
        _errorMessage = 'OCR解析中にエラーが発生しました: $e';
      });
    }
  }
                  ...
                  child: Crop(
                    ...
                    onCropped: (result) {
                      ...
                      if (result is CropSuccess) {
                        // 成功した場合
                        print('トリミング成功: ${result.croppedImage.length} bytes');
                        setState(() {
                          _isCropping = false;
                          ...
                          _showCropInterface = true;
                          _errorMessage = 'トリミング成功。画像が更新されました。';
                        });
                        ...
                        setState(() {
                          _isCropping = false;
                          _cropRequested = false; // トリミングリクエストを完了状態に設定
                          _errorMessage = 'トリミングに失敗しました: ${result.cause}';
                        });
                        ...
                        setState(() {
                          _isCropping = false;
                          _cropRequested = false; // トリミングリクエストを完了状態に設定
                          _errorMessage = 'トリミング結果の処理に問題がありました。';
                        });
                      }
                    },
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        ...
                        onPressed:
                            _isCropping || _isAnalyzing ? null : _cropImage,
                      ),
                    ),
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        ...
                        label: const Text('読み取り',
                            style: TextStyle(color: Colors.black)),
                        style: ElevatedButton.styleFrom(
                          padding: const EdgeInsets.symmetric(vertical: 12),
                          backgroundColor: _hasMathFormula || _isMathMode
                              ? const Color.fromARGB(255, 193, 121, 206)
                              : Colors.green.shade400,
                        ),
                        onPressed:
                            _isCropping || _isAnalyzing ? null : _executeOcr,
                      ),
                    ),
                  ],
                ),
